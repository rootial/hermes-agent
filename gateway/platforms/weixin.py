...
class WeixinAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if any(state.group_policy != "disabled" for state in self._accounts):
            logger.warning(
                "[%s] Weixin group policy is enabled, but QR-login connects an iLink bot "
                "identity which may not receive ordinary WeChat group events.",
                self.name,
            )
        return True
    ...
    async def _process_message(
        ...
        if text:
            content_key = f"content:{state.account_id}:{sender_id}:{hashlib.md5(text.encode()).hexdigest()}"
            if state.dedup.is_duplicate(content_key):
    ...
    async def _ensure_typing_ticket(
        ...
        if ticket:
            return ticket
        if not self._send_session or not state.token:
            return None
        ...
        try:
            ...
            typing_ticket = str(response.get("typing_ticket") or "")
            if typing_ticket:
                state.typing_cache.set(chat_id, typing_ticket)
