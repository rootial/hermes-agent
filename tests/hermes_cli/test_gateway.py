...
@pytest.mark.parametrize(
    ("drain_exited", "expected_kickstart"),
    [
        (True, ["launchctl", "kickstart", "gui/501/ai.hermes.gateway-test"]),
        (False, ["launchctl", "kickstart", "-k", "gui/501/ai.hermes.gateway-test"]),
    ],
)
def test_launchd_restart_kickstart_matches_drain_result(
    monkeypatch, drain_exited, expected_kickstart
):
    calls = []
    pid_reads = iter([12345, 54321])
    ...
    monkeypatch.setattr("gateway.status.get_running_pid", lambda: next(pid_reads, 54321))
    monkeypatch.setattr(gateway, "_request_gateway_self_restart", lambda pid: False)
    monkeypatch.setattr(gateway, "terminate_pid", lambda pid, force=False: None)
    monkeypatch.setattr(
        gateway, "_wait_for_gateway_exit", lambda timeout, force_after=None: drain_exited
    )
    monkeypatch.setattr(
        gateway.subprocess,
        "run",
    ...
    assert expected_kickstart in calls


def test_launchd_restart_forces_kickstart_when_pid_never_reappears(monkeypatch):
    calls = []
    pid_reads = iter([12345] + [None] * 10)

    monkeypatch.setattr(gateway, "get_launchd_label", lambda: "ai.hermes.gateway-test")
    monkeypatch.setattr(gateway, "_launchd_domain", lambda: "gui/501")
    monkeypatch.setattr(gateway, "_get_restart_drain_timeout", lambda: 42.0)
    monkeypatch.setattr("gateway.status.get_running_pid", lambda: next(pid_reads, None))
    monkeypatch.setattr(gateway, "_request_gateway_self_restart", lambda pid: False)
    monkeypatch.setattr(gateway, "terminate_pid", lambda pid, force=False: None)
    monkeypatch.setattr(
        gateway, "_wait_for_gateway_exit", lambda timeout, force_after=None: True
    )
    monkeypatch.setattr("time.sleep", lambda _: None)
    monkeypatch.setattr(
        gateway.subprocess,
    ...
    assert calls[-1] == [
        ...
        "kickstart",
        "-k",
        "gui/501/ai.hermes.gateway-test",
    ]
