...
class TestAtomicSnapshotWrite:
    ...
    def test_temp_path_uses_mktemp_not_shell_pid(self):
        """Each writer receives a unique path on Bash 3.2 and newer shells."""
        env = _TestableEnv()
        env._snapshot_ready = True
        wrapped = env._wrap_command("echo hi", "/tmp")
    ...
    def test_temp_path_template_is_quoted(self):
        """The mktemp template remains one shell word when paths contain spaces."""
        env = _TestableEnv()
        env._snapshot_ready = True
...
class TestAtomicSnapshotConcurrencyBehavioral:
    ...
    that would corrupt PATH. Each writer uses a path allocated by ``mktemp``.
    """
    ...
    def test_concurrent_writes_never_tear_the_snapshot(self, tmp_path):
        ...
        writer = (
            ...
            f"if __snap_tmp=$(mktemp {_snap_tmp_template}); then "
            f"{{ export -p > \"$__snap_tmp\" && mv -f \"$__snap_tmp\" {_q(snap)}; }} "
            f"2>/dev/null || rm -f \"$__snap_tmp\" 2>/dev/null || true; fi; "
            "done"
        )
